...
export default function DuplicateAlert({ signedIn }: Props) {
  ...
  if (loading) {
    return (
      <div className="relative overflow-hidden rounded-3xl border border-blue-100 dark:border-slate-800/50 bg-gradient-to-br from-blue-50 to-sky-50 dark:from-slate-900/50 dark:to-slate-950/50 p-8">
        <div className="relative flex items-center justify-center gap-4">
          <div className="w-12 h-12 border-3 border-blue-200 dark:border-slate-700 border-t-blue-500 rounded-full animate-spin" />
          <p className="text-lg text-slate-600 dark:text-slate-400 font-medium">Scanning your Google Drive...</p>
        </div>
      </div>
    )
  }
  ...
  if (clusters.length === 0) {
    return (
      <div className="relative overflow-hidden rounded-3xl border border-blue-500/20 bg-gradient-to-br from-blue-50 to-sky-50 dark:from-blue-950/30 dark:to-sky-950/30 p-8">
        <div className="relative flex items-center gap-6">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-400 to-sky-500 flex items-center justify-center shadow-xl shadow-blue-500/20">
            <span className="text-white text-3xl">✓</span>
          </div>
          <div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="relative">
          ...
          return (
            <div
              ...
              className={`
                ...
                bg-gradient-to-br ${clusterColor.gradient} ${clusterColor.border} shadow-2xl ${clusterColor.glow}
                ${isClusterMerged ? 'opacity-40' : ''}
              `}
            >
              <div className="flex items-start gap-4">
                <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${cluster.color === 'red' ? 'from-red-400 to-orange-500' : cluster.color === 'amber' ? 'from-amber-400 to-yellow-500' : 'from-blue-400 to-sky-500'} flex items-center justify-center shadow-xl ${cluster.color === 'red' ? 'shadow-red-500/20' : cluster.color === 'amber' ? 'shadow-amber-500/20' : 'shadow-blue-500/20'}`}>
                ...
                <div className="mt-auto space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="px-5 py-3 rounded-2xl text-sm font-medium border border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
                    >
                      Skip
                    </button>
                  </div>
        ...
        <button
          onClick={prevCluster}
          className="absolute left-0 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white dark:bg-slate-800 border border-blue-100 dark:border-slate-700 flex items-center justify-center shadow-xl hover:scale-110 transition-all z-20 text-lg"
        >
          ←
        </button>
        <button
          onClick={nextCluster}
          className="absolute right-0 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white dark:bg-slate-800 border border-blue-100 dark:border-slate-700 flex items-center justify-center shadow-xl hover:scale-110 transition-all z-20 text-lg"
        >
          →
        </button>
      </div>
      ...
      {/* Differences section */}
      {clusters[currentIndex]?.differences && clusters[currentIndex].differences!.length > 0 && (
        <div className="max-w-3xl mx-auto mt-6">
          ...
          <div className="space-y-2">
            {clusters[currentIndex].differences!.map((diff, diffIndex) => (
              <div
                key={diffIndex}
                className="bg-white dark:bg-slate-900 rounded-xl border border-blue-100 dark:border-slate-800 p-4"
              >
                ...
                <div className="space-y-1">
                  {diff.details.map((detail, detailIndex) => (
                    <div
                      key={detailIndex}
                      className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 py-1 border-b border-blue-50 dark:border-slate-800 last:border-0"
                    >
